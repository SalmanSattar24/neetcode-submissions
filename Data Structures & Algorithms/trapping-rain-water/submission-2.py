class Solution:
    def trap(self, height: List[int]) -> int:
        ...
        in a given elevation map. The elevation map is represented by a list of
        non-negative integers where each integer represents the height of a bar
        and the width of each bar is 1.

        This solution uses a dynamic programming approach where two auxiliary arrays
        are pre-calculated to store the maximum heights to the left and right of
        each bar.
        ...
        Returns:
            int: The total volume of trapped rainwater.

        Time Complexity: O(N)
            - Three passes over the array of length N:
                - One pass to build `max_left_wall`.
                - One pass to build `max_right_wall`.
                - One pass to calculate the `total_trapped_water`.
            Each pass takes O(N) time. Therefore, the overall time complexity is O(N).

        Space Complexity: O(N)
            - Two auxiliary arrays (`max_left_wall` and `max_right_wall`) are created,
              each of size N.
